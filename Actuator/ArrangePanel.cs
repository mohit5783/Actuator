...
namespace Actuator
{
    ...
    {
        private Duration duration = new Duration(new TimeSpan(0, 0, 0, 0, 300));
        ...
        protected override Size MeasureOverride(Size availableSize)
        {
            ...
            double childCount = this.Children.Count;
            ...
            double totalHeight = (ItemSize + ItemMargin) * (Math.Ceiling(childCount / childrenPerRow));
        ...
        private void ArrangeChild(FrameworkElement element, Point finalPosition, Size availableSize)
        {
            ...
            double randomX = 0;// CrazyMode ? random.Next(0, (int)availableSize.Width) : 0;
            double randomY = 0;// CrazyMode ? random.Next(0, (int)availableSize.Height) : 0;
        ...
        private void BeginAnimation(FrameworkElement element, Point translatePosition)
        {
            ...
            element.RenderTransform.BeginAnimation(TranslateTransform.XProperty, new DoubleAnimation(translatePosition.X, duration) { EasingFunction = new CubicEase() { EasingMode = EasingMode.EaseOut } }, HandoffBehavior.Compose);
            element.RenderTransform.BeginAnimation(TranslateTransform.YProperty, new DoubleAnimation(translatePosition.Y, duration) { EasingFunction = new CubicEase() { EasingMode = EasingMode.EaseOut } }, HandoffBehavior.Compose);
        }
    }
}
